...
namespace Namely
{
  ...
  public class MergeOverlappingIntervals
  {
    ...
    {1, 3},{2, 6},{4, 5},{7, 8},{8, 10},{15, 18}=> {1,6},{7,10},{15,18}*/
    public static List<List<int>> Calculate(List<List<int>> intervals)
    {
      ...
      for (int counter= 1; counter < intervals.Count; counter++)
      {
        ...
        var start = intervalInStack[0];
        var end=intervalInStack[1];
        if (end>=intervals[counter][0])
        {
          stack.Pop();
          stack.Push(new List<int> {start, Math.Max(intervals[counter][1],end)});
        }
        else
        {
